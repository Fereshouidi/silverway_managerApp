...
import { OrderType, DeliveryWorkerType } from '@/types';
    ...
    <SafeAreaView className="flex-1 bg-[#f8f9fa]">
      ...
      >
        ...
        <View className="mb-6 flex-row justify-between items-center">
          ...
          <TouchableOpacity onPress={() => setShowDate(!showDate)} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100">
        ...
        {showDate && 
          <View>
            ...
            <View className="flex-row bg-white rounded-xl p-2 mb-2 border border-gray-100 shadow-sm">
            ...
            <View className="mb-6">
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
                ...
                <TouchableOpacity
                  ...
                  className={`px-5 py-3 rounded-xl border shadow-sm flex-row items-center mr-3 ${activePreset === 'all' ? 'bg-orange-50 border-orange-200' : 'bg-white border-gray-100'}`}
                >
                ...
                <TouchableOpacity
                  ...
                  className={`px-5 py-3 rounded-xl border shadow-sm flex-row items-center mr-3 ${activePreset === 'week' ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-100'}`}
                >
                ...
                <TouchableOpacity
                  ...
                  className={`px-5 py-3 rounded-xl border shadow-sm flex-row items-center mr-3 ${activePreset === '2weeks' ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-100'}`}
                >
                ...
                <TouchableOpacity
                  ...
                  className={`px-5 py-3 rounded-xl border shadow-sm flex-row items-center mr-3 ${activePreset === 'month' ? 'bg-purple-50 border-purple-200' : 'bg-white border-gray-100'}`}
                >
                ...
                <TouchableOpacity
                  ...
                  className={`px-5 py-3 rounded-xl border shadow-sm flex-row items-center mr-3 ${activePreset === 'year' ? 'bg-rose-50 border-rose-200' : 'bg-white border-gray-100'}`}
                >
        ...
        <View className="flex-row flex-wrap justify-between">
          {stats.map((item) => (
            ...
            >
              <View className="bg-gray-50 self-start p-2 rounded-xl mb-3">{item.icon}</View>
        ...
        <View className="bg-white rounded-xl p-4 mb-10 border border-gray-100 shadow-sm">
          {topProducts.length > 0 ? (
            topProducts.map((product, i) => (
              ...
              >
                {/* Thumbnail Image */}
                <View className="w-14 h-14 bg-gray-100 rounded-xl mr-4 overflow-hidden border border-gray-50 items-center justify-center">
                  {product.image ? (
                    <Image
      ...
      >
        <View className="flex-1 bg-black/60 justify-end">
          ...
          />
          <View style={{ height: '80%', backgroundColor: colors.light[100] }} className="rounded-t-xl shadow-2xl overflow-hidden">
            <View className="p-6">
              <View className="w-12 h-1.5 bg-gray-200 rounded-full mb-6 self-center" />
              <Text className="text-2xl font-black text-black">Top Performers</Text>
            </View>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <View className="w-16 h-16 bg-gray-100 rounded-xl mr-4 overflow-hidden border border-gray-50">
                    {item.image ? (
      ...
      >
        <View className="flex-1 bg-black/60 justify-end">
          ...
          />
          <View style={{ height: '85%', backgroundColor: '#F8F9FA' }} className="rounded-t-xl shadow-2xl overflow-hidden">
            <View className="p-6 pb-2">
              ...
              <View className="flex-row justify-between items-center mb-4">
                ...
                <View className="bg-white px-4 py-2 rounded-xl border border-gray-100">
            ...
            {loadingMetric ? (
              ...
              <FlatList
                ...
                removeClippedSubviews={false}
                contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40 }}
                ListFooterComponent={loadingMoreMetric ? (
                  <View className="py-6">
                ...
                renderItem={({ item }) => {
                  if (selectedMetric?.title === 'Total Sales' || selectedMetric?.title === 'Avg. Value') {
                    ...
                    return (
                      <View className="bg-white p-5 rounded-xl mb-3 border border-gray-50 shadow-sm">
                  ...
                  if (selectedMetric?.title === 'New Clients') {
                    return (
                      <TouchableOpacity
                        ...
                        className="bg-white p-4 rounded-xl mb-3 border border-gray-50 shadow-sm flex-row items-center"
                      >
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      className="bg-white p-4 rounded-xl mb-3 border border-gray-50 shadow-sm"
                    >
                      <View className="flex-row justify-between items-center mb-3">
                        <View className="flex-row items-center">
                      ...
                      <View className="flex-row items-center">
                        ...
                        <Text className="text-md font-black text-black">
                          {(() => {
                            const subTotal = item.purchases?.reduce((acc: number, p: any) => {
                              const basePrice = p.specification?.price || p.specPrice || 0;
                              const charmsPrice = p.customizedCharms?.reduce((cAcc: number, pc: any) => {
                                const charmPrice = pc.spec?.price || pc.charm?.price || pc.charm?.specifications?.[0]?.price || 0;
                                return cAcc + charmPrice;
                              }, 0) || 0;
                              return acc + (basePrice + charmsPrice) * (p.quantity || 1);
                            }, 0) || 0;
                            return (subTotal + (item.shippingCoast || 0)).toLocaleString();
                          })()} DT
                        </Text>
                      </View>
                    </TouchableOpacity>
